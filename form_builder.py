...
function onTimeTrigger() {{
  ...
  for (var i = 1; i < data.length; i++) {{
    ...
    if (now >= expiry && row[notifiedCol] !== "YES") {{
      ...
      var meetLink = {json.dumps(meet_link)};
  ...
  if (allSlotsExpired) {{
    ...
    for (var j = 0; j < triggers.length; j++) {{
      if (triggers[j].getHandlerFunction() === "onTimeTrigger") {{
...
function refreshSlots() {{
  ...
  var limits = {slot_limits_js};
  var form = FormApp.openByUrl({json.dumps(form_edit_url)});
